        ...
        <div style={{padding:'14px 16px 14px'}}>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:7,borderTop:`1px solid ${G.border}`,paddingTop:10}}>
            ...
            <div style={{display:'flex',gap:5,alignItems:'center'}}>
              {score!=null&&<div style={{display:'flex',alignItems:'center',gap:8,flex:1}}><div style={{flex:1,height:7,background:'rgba(14,22,40,0.09)',borderRadius:4,overflow:'hidden',boxShadow:'inset 0 1px 2px rgba(14,22,40,0.08)'}}><div style={{height:'100%',width:`${score}%`,borderRadius:4,transition:'width 0.8s cubic-bezier(0.4,0,0.2,1)',background:score>=80?`linear-gradient(90deg,#22c55e,#4ade80)`:score>=50?`linear-gradient(90deg,${G.blue},#5b8ff0)`:`linear-gradient(90deg,#94a3b8,#cbd5e1)`,boxShadow:score>=80?'0 0 6px rgba(74,222,128,0.40)':score>=50?`0 0 6px rgba(36,88,212,0.35)`:'none'}}/></div><span style={{fontSize:11,fontWeight:800,color:score>=80?'#16a34a':score>=50?G.blue:'#64748b',minWidth:34,textAlign:'right'}}>{score}%</span></div>}
              <button style={{padding:'5px 11px',fontSize:11,fontWeight:600,borderRadius:6,cursor:'pointer',transition:'all 0.16s',background:isSp?hexToRgba(sc,0.10):'rgba(14,22,40,0.07)',color:isSp?sc:G.text,border:`1px solid ${isSp?hexToRgba(sc,0.28):G.border}`}} onClick={e=>{e.stopPropagation();onContact?.(p)}}>{t.contact}</button>
            </div>
          </div>
        </div>
      </div>
...
function Directory2({lang,t,profiles}){
  ...
  const results=useMemo(()=>{
    ...
    if(matchMode&&skills.length){r=r.map(p=>({...p,_score:calcMatch(p,skills)||0})).filter(p=>p._score>0).sort((a,b)=>b._score-a._score);return r}
    return r
    ...
    <div style={{background:'#FFFFFF',minHeight:'100vh',paddingTop:62}}>
      ...
      <div className="wrap2" style={{paddingTop:22,paddingBottom:48}}>
        ...
        <div style={{display:'flex',gap:8,marginBottom:10,flexWrap:'wrap',alignItems:'center'}}>
          ...
          <div style={{display:'flex',gap:3,background:'rgba(14,22,40,0.05)',padding:3,borderRadius:8,border:`1px solid ${G.border}`,flexShrink:0}}>
        ...
        <div style={{marginBottom:8,position:'relative'}}>
          ...
          {sectorOpen&&(
            <div style={{position:'absolute',top:'calc(100% + 4px)',left:0,right:0,zIndex:50,background:'#FFFFFF',border:`1px solid ${G.border}`,borderRadius:10,boxShadow:'0 8px 28px rgba(14,22,40,0.14)',padding:6,maxHeight:320,overflowY:'auto'}}>
              <button onClick={()=>{setCat('');setSectorOpen(false)}}
                style={{display:'flex',alignItems:'center',gap:8,width:'100%',padding:'9px 12px',background:!cat?hexToRgba(G.blue,0.07):'transparent',border:'none',borderRadius:7,fontSize:13,fontWeight:600,color:!cat?G.blue:G.text,cursor:'pointer',textAlign:'left',marginBottom:3}}>
                All Sectors {!cat&&<span style={{marginLeft:'auto',color:G.blue,fontSize:12}}>✓</span>}
              </button>
        ...
        <div style={{marginBottom:14}}>
          ...
          {matchOpen&&(
            <div style={{background:'#FFFFFF',border:`1.5px solid ${G.blueBd}`,borderTop:'none',borderRadius:'0 0 8px 8px',padding:'12px 14px',boxShadow:'0 6px 20px rgba(14,22,40,0.10)'}}>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                <input className="inp2" placeholder="Type extra skills… (comma separated)"
                  value={skillInput} onChange={e=>{setSkillInput(e.target.value);if(e.target.value.trim())setMatchMode(true)}}
                ...
                <button onClick={()=>{setMatchMode(v=>!v);if(matchMode){setSelSkills([]);setSkillInput('')}}}
                  ...
                  {matchMode?'✓ Active':'Activate'}
                </button>
              </div>
              <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
                {allTags.map(tag=>(
                  <button key={tag} onClick={()=>{toggleSkill(tag);setMatchMode(true)}}
                    style={{padding:'4px 10px',borderRadius:20,border:`1.5px solid ${selSkills.includes(tag)?hexToRgba(G.blue,0.45):G.border}`,background:selSkills.includes(tag)?hexToRgba(G.blue,0.08):'transparent',color:selSkills.includes(tag)?G.blue:G.muted,fontSize:11,fontWeight:600,cursor:'pointer',transition:'all 0.13s'}}>
                    {tag}
                  </button>
                ))}
              </div>
...
function Concierge2({lang,t,siteContent,partnerProfiles=[]}){
  ...
  return(<div style={{background:'#FFFFFF',minHeight:'100vh',paddingTop:62}}>
    ...
    <div className="wrap2" style={{paddingTop:44,paddingBottom:48}}>
      ...
      {partnerProfiles.length>0&&<div style={{marginTop:8}}><div className="sec-label">Network</div><h2 style={{fontWeight:700,fontSize:22,marginBottom:18,letterSpacing:'-0.3px',color:G.text}}>Our Partner Network</h2><div style={{display:'flex',flexDirection:'column',gap:10}}>{partnerProfiles.map(p=><PartnerCard2 key={p.id} p={p} lang={lang} t={t} onView={p2=>setPartnerDetail(p2)}/>)}</div></div>}
    </div>
  ...
  {partnerDetail&&<DetailModal2 p={partnerDetail} lang={lang} t={t} onClose={()=>setPartnerDetail(null)} onContact={()=>setPartnerDetail(null)}/>}
  </div>)
}
