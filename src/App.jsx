...
const G = {
  bg:'#0f1117', surface:'#171a22', card:'#1a1d28', navBg:'#0E1628',
  text:'#E2DDD6', muted:'rgba(226,221,214,0.50)', dim:'rgba(226,221,214,0.30)',
  border:'rgba(226,221,214,0.09)',
  blue:'#2458D4', blueDim:'rgba(36,88,212,0.09)', blueBd:'rgba(36,88,212,0.25)',
...
html,body,#root{font-family:'Plus Jakarta Sans',sans-serif;background:#0f1117;color:#E2DDD6;-webkit-font-smoothing:antialiased;overflow-x:hidden;}
...
.btn-ghost2{background:transparent;color:rgba(226,221,214,0.52);border:1px solid rgba(226,221,214,0.12);padding:7px 13px;border-radius:7px;font-size:12px;font-weight:500;transition:all 0.16s;}
.btn-ghost2:hover{color:#E2DDD6;border-color:rgba(226,221,214,0.28);}
.inp2{background:rgba(255,255,255,0.05);border:1.5px solid rgba(226,221,214,0.12);border-radius:8px;padding:10px 14px;color:#E2DDD6;width:100%;font-size:14px;outline:none;transition:border 0.18s;}
.inp2:focus{border-color:#2458D4;box-shadow:0 0 0 3px rgba(36,88,212,0.12);}
.inp2::placeholder{color:rgba(226,221,214,0.28);}
.label2{display:block;font-size:10px;font-weight:700;letter-spacing:0.8px;text-transform:uppercase;color:rgba(226,221,214,0.38);margin-bottom:6px;}
...
.modal-bg2{position:fixed;inset:0;background:rgba(5,8,18,0.84);z-index:200;display:flex;align-items:center;justify-content:center;padding:14px;backdrop-filter:blur(8px);}
.modal2{background:#171a22;border:1px solid rgba(226,221,214,0.10);border-radius:14px;padding:24px;max-width:520px;width:100%;max-height:92vh;overflow-y:auto;box-shadow:0 24px 64px rgba(0,0,0,0.55);}
...
::-webkit-scrollbar{width:4px;}::-webkit-scrollbar-thumb{background:rgba(36,88,212,0.28);border-radius:3px;}
...
function Card2({p,lang,t,onContact,onView,score}){
  ...
  return(
    <div style={{position:'relative',paddingTop:isSp?14:0}} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}>
      ...
      <div onClick={()=>onView?.(p)} style={{background:isSp?`linear-gradient(160deg,${hexToRgba(sc,0.14)} 0%,#1a1d2e 65%)`:(hov?'#1e2135':'#181c2c'),border:isSp?`1px solid ${hexToRgba(sc,0.40)}`:`1px solid ${hov?hexToRgba(sc,0.28):'rgba(255,255,255,0.07)'}`,borderTop:`3px solid ${sc}`,borderRadius:12,overflow:'hidden',cursor:'pointer',position:'relative',boxShadow:isSp?`0 8px 32px rgba(0,0,0,0.34),0 0 0 1px ${hexToRgba(sc,0.12)}`:(hov?'0 8px 28px rgba(0,0,0,0.30)':'0 2px 12px rgba(0,0,0,0.20)'),transition:'all 0.22s cubic-bezier(0.4,0,0.2,1)'}}>
        {p.coverImage&&<div style={{height:64,overflow:'hidden',position:'relative'}}><img src={p.coverImage} alt="" style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:p.coverFocus||'50% 50%'}}/><div style={{position:'absolute',inset:0,background:'linear-gradient(0deg,#181c2c 0%,transparent 60%)'}}/></div>}
        <div style={{padding:'14px 16px 14px'}}>
          <div style={{display:'flex',gap:10,alignItems:'flex-start',marginBottom:9}}>
            ...
            <div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,fontSize:14,color:'#F0EFEE',lineHeight:1.2,marginBottom:2}}>{p.name}</div><div style={{fontSize:11,color:'rgba(240,239,238,0.42)',display:'flex',gap:5,flexWrap:'wrap'}}>{p.city&&<span>📍 {p.city}</span>}{p.employees&&<span>· 👥 {p.employees}</span>}</div></div>
          </div>
          {desc&&<p style={{fontSize:12,color:'rgba(240,239,238,0.52)',lineHeight:1.65,marginBottom:8}}>{desc}{(p.desc?.[lang]||p.desc?.en||'').length>112?'…':''}</p>}
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:7,borderTop:'1px solid rgba(255,255,255,0.07)',paddingTop:10}}>
...
function DetailModal2({p,lang,t,onClose,onContact}){
  if(!p)return null
  const sc=catColor(p.cat)
  return(<div className="modal-bg2" onClick={e=>e.target===e.currentTarget&&onClose()}><div className="modal2" style={{maxWidth:570,padding:0,borderRadius:14,overflow:'hidden'}}><div style={{height:5,background:`linear-gradient(90deg,${sc},${G.blue})`}}/>{p.coverImage&&<div style={{height:85,overflow:'hidden',position:'relative'}}><img src={p.coverImage} alt="" style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:p.coverFocus||'50% 50%'}}/><div style={{position:'absolute',inset:0,background:'linear-gradient(0deg,#171a22 0%,transparent 55%)'}}/></div>}<div style={{padding:'18px 21px 0'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}><div style={{display:'flex',gap:11,alignItems:'center'}}><Lg name={p.logo||p.name} color={sc} url={p.logoUrl} size={46}/><div><div style={{display:'flex',gap:5,alignItems:'center',flexWrap:'wrap',marginBottom:3}}><span style={{fontWeight:700,fontSize:17,color:'#F0EFEE'}}>{p.name}</span>{p.verified&&<span style={{fontSize:9,fontWeight:700,color:'#4ade80',padding:'2px 6px',background:'rgba(74,222,128,0.08)',border:'1px solid rgba(74,222,128,0.22)',borderRadius:20}}>✓ {t.verified}</span>}</div><div style={{fontSize:12,color:G.muted,display:'flex',gap:7,flexWrap:'wrap'}}><span style={{padding:'2px 6px',background:hexToRgba(sc,0.10),color:sc,border:`1px solid ${hexToRgba(sc,0.20)}`,borderRadius:5,fontSize:10,fontWeight:600}}>{catIcon(p.cat)} {catLabel(p.cat,lang)}</span>{p.city&&<span>📍 {p.city}</span>}{p.employees&&<span>👥 {p.employees}</span>}</div></div></div><button onClick={onClose} className="btn-ghost2">✕</button></div></div><div style={{padding:'17px 21px',maxHeight:'52vh',overflowY:'auto'}}>{(p.desc?.[lang]||p.desc?.en)&&<p style={{color:G.muted,lineHeight:1.72,fontSize:13,marginBottom:13}}>{p.desc?.[lang]||p.desc?.en}</p>}{p.tags?.length>0&&<div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:13}}>{p.tags.map(tag=><span key={tag} style={{padding:'2px 6px',borderRadius:4,fontSize:11,background:hexToRgba(sc,0.10),color:sc,border:`1px solid ${hexToRgba(sc,0.18)}`}}>{tag}</span>)}</div>}{p.website&&<div style={{marginBottom:9}}><label className="label2">Website</label><a href={`https://${p.website.replace(/^https?:\/\//,'')}`} target="_blank" rel="noopener noreferrer" style={{color:G.blue,fontSize:13,fontWeight:500}}>{p.website} →</a></div>}</div><div style={{padding:'13px 21px',borderTop:'1px solid rgba(255,255,255,0.07)',display:'flex',justifyContent:'flex-end',gap:8}}>{p.website&&<a href={`https://${p.website.replace(/^https?:\/\//,'')}`} target="_blank" rel="noopener noreferrer" className="btn-ghost2" style={{padding:'7px 14px',fontSize:12}}>Website →</a>}<button className="btn-primary2" onClick={()=>{onClose();onContact?.(p)}}>{t.contact}</button></div></div></div>)
}
...
function RegModal2({t,lang,onClose}){
  ...
  {step===0&&<div style={{display:'flex',flexDirection:'column',gap:9}}>{[['company','🏢','Company / Agency','Register your company or agency'],['freelancer','👤','Freelancer / Consultant','Solo professional listing'],['partner','🤝','Partner / Institution','Network or institution']].map(([id,ic,label,sub])=><button key={id} onClick={()=>{setType(id);setStep(1)}} style={{display:'flex',alignItems:'center',gap:13,padding:'14px 16px',background:'rgba(255,255,255,0.03)',border:'1.5px solid rgba(226,221,214,0.10)',borderRadius:10,textAlign:'left',color:G.text,transition:'all 0.18s'}} onMouseEnter={e=>{e.currentTarget.style.borderColor=G.blue;e.currentTarget.style.background=G.blueDim}} onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(226,221,214,0.10)';e.currentTarget.style.background='rgba(255,255,255,0.03)'}}><span style={{fontSize:21}}>{ic}</span><div><div style={{fontWeight:600,fontSize:13}}>{label}</div><div style={{fontSize:12,color:G.muted}}>{sub}</div></div><span style={{marginLeft:'auto',color:G.blue}}>→</span></button>)}</div>}
  {step===1&&<div><div style={{fontWeight:600,fontSize:13,marginBottom:13,color:G.muted}}>Choose your listing plan</div><div style={{display:'flex',flexDirection:'column',gap:9,marginBottom:16}}>
    <button onClick={()=>{setTier('free');setStep(2)}} style={{padding:'15px 16px',background:tier==='free'?G.blueDim:'rgba(255,255,255,0.03)',border:`2px solid ${tier==='free'?G.blue:'rgba(226,221,214,0.10)'}`,borderRadius:10,textAlign:'left',cursor:'pointer'}}>
      <div style={{display:'flex',justifyContent:'space-between',marginBottom:5}}><span style={{fontWeight:700,fontSize:14,color:'#F0EFEE'}}>Free Listing</span><span style={{fontWeight:800,fontSize:17,color:'#4ade80'}}>Free</span></div>
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:5,marginTop:8}}>{['Name & city','Sector','Description','Tags & skills','Contact button'].map(f=><span key={f} style={{fontSize:10,color:G.muted,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:4,padding:'2px 6px'}}>✓ {f}</span>)}</div>
    </button>
    <button onClick={()=>{setTier('sponsored');setStep(2)}} style={{padding:'15px 16px',background:tier==='sponsored'?G.blueDim:'rgba(255,255,255,0.03)',border:`2px solid ${tier==='sponsored'?G.blue:'rgba(226,221,214,0.10)'}`,borderRadius:10,textAlign:'left',cursor:'pointer',position:'relative'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',marginBottom:5}}><span style={{fontWeight:700,fontSize:14,color:'#F0EFEE'}}>Sponsored Listing</span><span style={{fontWeight:800,fontSize:17,color:G.blue}}>€49/mo</span></div>
...
function Home2({lang,t,profiles,setPage,onReg}){
  ...
  return(<div>
    ...
    <div style={{background:G.bg}}>
      {sponsored.length>0&&<div className="section2 wrap2"><div className="sec-label">{t.featuredTitle}</div><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',marginBottom:18}}><h2 style={{fontWeight:700,fontSize:22,letterSpacing:'-0.4px',color:'#F0EFEE'}}>{t.featuredTitle}</h2><button className="btn-ghost2" onClick={()=>setPage('directory')}>{t.viewAll}</button></div><div className="grid-3">{sponsored.map(p=><Card2 key={p.id} p={p} lang={lang} t={t} onContact={setContact} onView={setDetail}/>)}</div></div>}
      {partners.length>0&&<div style={{paddingBottom:48,borderBottom:'1px solid rgba(255,255,255,0.06)'}}><div className="wrap2" style={{marginBottom:14}}><div className="sec-label">Official Partners</div></div><div className="ticker2"><div className="ticker2-track">{[...partners,...partners].map((p,i)=><div key={i} style={{display:'flex',alignItems:'center',gap:9,padding:'9px 14px',background:G.surface,border:'1px solid rgba(255,255,255,0.07)',borderRadius:9,flexShrink:0,minWidth:162}}><Lg name={p.logo||p.name} color={catColor(p.cat)} url={p.logoUrl} size={30}/><div><div style={{fontWeight:600,fontSize:12,color:'#F0EFEE'}}>{p.name}</div><div style={{fontSize:10,color:G.dim}}>{p.city||catLabel(p.cat,'en')}</div></div></div>)}</div></div></div>}
      <div className="section2 wrap2"><div className="sec-label">Sectors</div><h2 style={{fontWeight:700,fontSize:22,marginBottom:18,letterSpacing:'-0.4px',color:'#F0EFEE'}}>Browse by Sector</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))',gap:9}}>{CATS.map(cat=><button key={cat.id} onClick={()=>setPage('directory')} style={{display:'flex',alignItems:'center',gap:9,padding:'12px 14px',background:G.surface,border:'1px solid rgba(255,255,255,0.07)',borderRadius:9,textAlign:'left',transition:'all 0.18s',cursor:'pointer'}} onMouseEnter={e=>{e.currentTarget.style.borderColor=hexToRgba(cat.color,0.42);e.currentTarget.style.background=hexToRgba(cat.color,0.08);e.currentTarget.style.transform='translateY(-2px)'}} onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(255,255,255,0.07)';e.currentTarget.style.background=G.surface;e.currentTarget.style.transform=''}}><div style={{width:32,height:32,borderRadius:8,background:hexToRgba(cat.color,0.14),border:`1px solid ${hexToRgba(cat.color,0.22)}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,flexShrink:0}}>{cat.icon}</div><span style={{fontWeight:600,fontSize:12,color:'#F0EFEE',lineHeight:1.25}}>{cat.labels[lang]||cat.labels.en}</span></button>)}</div></div>
      <div style={{background:G.navBg,padding:'48px 0',borderTop:'1px solid rgba(255,255,255,0.06)'}}><div className="wrap2" style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:22,flexWrap:'wrap'}}><div><div className="sec-label" style={{color:'#93B4F8'}}>For Businesses</div><h3 style={{fontWeight:700,fontSize:22,color:'#F0EFEE',marginBottom:7,letterSpacing:'-0.3px'}}>Is your business on Kosova Hub?</h3><p style={{color:'rgba(240,239,238,0.48)',fontSize:13,maxWidth:400}}>Get discovered by international companies. Free for 6 months — no credit card needed.</p></div><div style={{display:'flex',gap:9,flexWrap:'wrap',flexShrink:0}}><button className="btn-white2" style={{padding:'11px 24px'}} onClick={onReg}>{t.registerBtn}</button><button className="btn-outline2" style={{color:'rgba(240,239,238,0.76)',borderColor:'rgba(240,239,238,0.18)',padding:'10px 18px'}} onClick={()=>setPage('concierge')}>Learn more →</button></div></div></div>
      <footer style={{borderTop:'1px solid rgba(255,255,255,0.06)',padding:'20px 0'}}><div className="wrap2" style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:9}}><span style={{fontSize:12,color:G.dim}}>{t.footer}</span><div style={{display:'flex',gap:14}}>{['Privacy','Terms','Contact'].map(l=><a key={l} href="#" style={{fontSize:12,color:G.dim}}>{l}</a>)}</div></div></footer>
    </div>
...
function Directory2({lang,t,profiles}){
  ...
  const allTags=useMemo(()=>{const counts={};base.forEach(p=>(p.tags||[]).forEach(t=>{counts[t]=(counts[t]||0)+1}));return Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,22).map(([t])=>t)},[base])
  ...
  const grouped=useMemo(()=>{if(matchMode&&skills.length)return null;const g={};results.forEach(p=>{const key=p.cat||'other';if(!g[key])g[key]=[];g[key].push(p)});return g},[results,matchMode,skills])
  return(<div style={{background:G.bg,minHeight:'100vh',paddingTop:62}}>
    <div style={{background:G.navBg,padding:'48px 0 36px',position:'relative',overflow:'hidden'}}><VideoBg2 src="/bg-video-companies.mp4"/><div className="wrap2" style={{position:'relative',zIndex:1}}><div className="sec-label" style={{color:'#93B4F8'}}>Business Directory</div><h1 style={{fontWeight:800,fontSize:'clamp(28px,5vw,48px)',letterSpacing:'-1px',color:'#F0EFEE',marginBottom:7}}>Companies &amp; Freelancers</h1><p style={{color:'rgba(240,239,238,0.50)',fontSize:14}}>Verified businesses from Kosova — open to worldwide partnerships</p></div></div>
    <div className="wrap2" style={{paddingTop:24,paddingBottom:48}}>
      <div style={{display:'flex',gap:9,marginBottom:13,flexWrap:'wrap',alignItems:'center'}}><input className="inp2" placeholder={t.searchPH||'Search…'} value={q} onChange={e=>setQ(e.target.value)} style={{flex:1,minWidth:190,maxWidth:390}}/><div style={{display:'flex',gap:3,background:'rgba(255,255,255,0.04)',padding:3,borderRadius:8,border:'1px solid rgba(255,255,255,0.07)'}}>{[['all',t.allTypes],['company',t.onlyComp],['freelancer',t.onlyFL]].map(([v,l])=><button key={v} onClick={()=>setTypeF(v)} style={{padding:'5px 12px',borderRadius:6,border:'none',fontSize:12,fontWeight:600,background:typeF===v?G.blue:'transparent',color:typeF===v?'#fff':G.muted,cursor:'pointer',transition:'all 0.15s'}}>{l}</button>)}</div></div>
      <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:13}}><button onClick={()=>setCat('')} style={{padding:'4px 11px',borderRadius:20,border:`1.5px solid ${!cat?G.blue:'rgba(255,255,255,0.10)'}`,background:!cat?G.blueDim:'transparent',color:!cat?G.blue:G.muted,fontSize:11,fontWeight:600,cursor:'pointer'}}>{t.allSectors}</button>{CATS.map(c=><button key={c.id} onClick={()=>setCat(cat===c.id?'':c.id)} style={{padding:'4px 10px',borderRadius:20,border:`1.5px solid ${cat===c.id?hexToRgba(c.color,0.50):hexToRgba(c.color,0.18)}`,background:cat===c.id?hexToRgba(c.color,0.12):'transparent',color:cat===c.id?c.color:G.muted,fontSize:11,fontWeight:600,cursor:'pointer'}}>{c.icon} {c.labels[lang]||c.labels.en}</button>)}</div>
      <div style={{marginBottom:16,padding:'12px 14px',background:G.surface,border:`1.5px solid ${matchMode?G.blue:'rgba(255,255,255,0.08)'}`,borderRadius:10,transition:'border 0.18s'}}>
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:allTags.length?9:0}}><span style={{fontWeight:700,fontSize:12,color:matchMode?G.blue:G.muted,flexShrink:0}}>🎯 Skill Match</span><input className="inp2" placeholder="Type extra skills… (comma separated)" value={skillInput} onChange={e=>setSkillInput(e.target.value)} style={{flex:1,fontSize:12,padding:'5px 9px'}}/><button onClick={()=>{setMatchMode(v=>!v);if(matchMode){setSelSkills([]);setSkillInput('')}}} style={{padding:'5px 12px',borderRadius:7,border:`1.5px solid ${matchMode?G.blue:'rgba(255,255,255,0.10)'}`,background:matchMode?G.blue:'transparent',color:matchMode?'#fff':G.muted,fontSize:12,fontWeight:600,cursor:'pointer',flexShrink:0}}>{matchMode?'✓ Active':'Match'}</button></div>
        {allTags.length>0&&<div style={{display:'flex',gap:4,flexWrap:'wrap'}}>{allTags.map(tag=><button key={tag} onClick={()=>{toggleSkill(tag);setMatchMode(true)}} style={{padding:'3px 8px',borderRadius:20,border:`1.5px solid ${selSkills.includes(tag)?hexToRgba(G.blue,0.50):'rgba(255,255,255,0.10)'}`,background:selSkills.includes(tag)?G.blueDim:'transparent',color:selSkills.includes(tag)?G.blue:G.muted,fontSize:11,fontWeight:600,cursor:'pointer',transition:'all 0.13s'}}>{tag}</button>)}</div>}
      </div>
      <div style={{fontSize:12,color:G.dim,marginBottom:14}}>{results.length} listing{results.length!==1?'s':''} found{matchMode&&skills.length>0&&<span style={{color:G.blue,marginLeft:7}}>· Matching: {skills.join(', ')}</span>}</div>
      {results.length===0?(<div style={{textAlign:'center',padding:'56px 0',color:G.muted}}><div style={{fontSize:36,marginBottom:11}}>🔍</div><div style={{fontWeight:700,fontSize:18,marginBottom:5,color:'#F0EFEE'}}>{t.noResults}</div><div style={{fontSize:13}}>{t.noResultsSub}</div></div>)
        :matchMode&&skills.length>0?(<div className="grid-3">{results.map(p=><Card2 key={p.id} p={p} lang={lang} t={t} onContact={setContact} onView={setDetail} score={p._score}/>)}</div>)
        :grouped?(Object.entries(grouped).map(([catId,ps])=>{const cat2=CATS.find(c=>c.id===catId);if(!ps.length)return null;return(<div key={catId} style={{marginBottom:36}}><div style={{display:'flex',alignItems:'center',gap:9,marginBottom:13,paddingBottom:9,borderBottom:`2px solid ${hexToRgba(cat2?.color||G.blue,0.22)}`}}><span style={{fontSize:17}}>{cat2?.icon||'🏢'}</span><span style={{fontWeight:700,fontSize:14,color:'#F0EFEE'}}>{cat2?.labels[lang]||catId}</span><span style={{fontSize:12,color:G.dim}}>({ps.length})</span></div><div className="grid-3">{ps.map(p=><Card2 key={p.id} p={p} lang={lang} t={t} onContact={setContact} onView={setDetail}/>)}</div></div>)})):null}
    </div>
    {detail&&<DetailModal2 p={detail} lang={lang} t={t} onClose={()=>setDetail(null)} onContact={p=>{setDetail(null);setContact(p)}}/>}
    {contact&&<EnquiryModal2 target={contact} t={t} onClose={()=>setContact(null)}/>}
  </div>)
...
function Concierge2({lang,t,siteContent,partnerProfiles=[]}){
  ...
  return(<div style={{background:G.bg,minHeight:'100vh',paddingTop:62}}>
    ...
    <div className="wrap2" style={{paddingTop:44,paddingBottom:48}}>
      ...
      <h2 style={{fontWeight:700,fontSize:22,marginBottom:20,letterSpacing:'-0.3px',color:'#F0EFEE'}}>Our General Partners</h2>
      <div className="grid-2" style={{marginBottom:48}}>
        <div style={{background:G.surface,border:'1px solid rgba(255,255,255,0.08)',borderTop:`3px solid ${G.blue}`,borderRadius:12,overflow:'hidden'}}>
          ...
          <div style={{padding:'17px'}}><div style={{display:'flex',alignItems:'center',gap:10,marginBottom:11}}>{P.rootsgtm_logo?<img src={P.rootsgtm_logo} alt="" style={{width:40,height:40,borderRadius:8,objectFit:'cover',border:'1.5px solid rgba(255,255,255,0.10)'}}/>:<div style={{width:40,height:40,borderRadius:8,background:G.blueDim,border:`1.5px solid ${G.blueBd}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:19}}>🚀</div>}<div><div style={{fontWeight:700,fontSize:15,color:'#F0EFEE'}}>{P.rootsgtm_name||'rootsGTM'}</div><div style={{fontSize:11,color:G.dim}}>General Partner · Active</div></div><span style={{marginLeft:'auto',fontSize:10,fontWeight:700,color:'#4ade80',padding:'2px 6px',background:'rgba(74,222,128,0.08)',border:'1px solid rgba(74,222,128,0.22)',borderRadius:20}}>✓ Live</span></div><p style={{fontSize:12,color:G.muted,lineHeight:1.65,marginBottom:12}}>{P.rootsgtm_desc||'rootsGTM is our exclusive sales and business development network connecting EU businesses with Kosova.'}</p><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:5,marginBottom:12}}>{['🤝 Direct contact','📅 Meeting setup','🎤 Events','📄 Follow-up'].map(f=><div key={f} style={{fontSize:11,color:G.muted,background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:5,padding:'5px 8px'}}>{f}</div>)}</div><button className="btn-primary2" style={{width:'100%'}} onClick={()=>setBookModal(true)}>Enquire via rootsGTM →</button></div>
        </div>
        <div style={{background:G.surface,border:'1px solid rgba(255,255,255,0.08)',borderTop:'3px solid #D97706',borderRadius:12,overflow:'hidden'}}>
          ...
          <div style={{padding:'17px'}}><div style={{display:'flex',alignItems:'center',gap:10,marginBottom:11}}>{P.gov_logo?<img src={P.gov_logo} alt="" style={{width:40,height:40,borderRadius:8,objectFit:'cover',border:'1.5px solid rgba(255,255,255,0.10)'}}/>:<div style={{width:40,height:40,borderRadius:8,background:'rgba(217,119,6,0.12)',border:'1.5px solid rgba(217,119,6,0.25)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:19}}>🏛️</div>}<div><div style={{fontWeight:700,fontSize:15,color:'#F0EFEE'}}>{P.gov_name||'Kosova Government'}</div><div style={{fontSize:11,color:G.dim}}>InvestKosova · Official Partner</div></div><span style={{marginLeft:'auto',fontSize:10,fontWeight:700,color:'#B45309',padding:'2px 6px',background:'rgba(217,119,6,0.10)',border:'1px solid rgba(217,119,6,0.22)',borderRadius:20}}>⏳ Negotiating</span></div><p style={{fontSize:12,color:G.muted,lineHeight:1.65,marginBottom:12}}>{P.gov_desc||'Building an official partnership with InvestKosova and the Ministry of Economy for investment support.'}</p><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:5,marginBottom:12}}>{['🏛️ InvestKosova','📋 Formation help','🤝 Gov meetings','📊 Investment support'].map(f=><div key={f} style={{fontSize:11,color:G.muted,background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:5,padding:'5px 8px'}}>{f}</div>)}</div><button onClick={()=>setBookModal(true)} style={{width:'100%',padding:'9px',borderRadius:8,border:'2px solid rgba(217,119,6,0.28)',background:'rgba(217,119,6,0.08)',color:'#B45309',fontWeight:700,fontSize:12,cursor:'pointer'}}>Request Government Meeting →</button></div>
        </div>
      </div>
      <div className="sec-label">Pricing</div>
      <h2 style={{fontWeight:700,fontSize:22,marginBottom:6,letterSpacing:'-0.3px',color:'#F0EFEE'}}>Concierge Packages</h2>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(255px,1fr))',gap:13,marginBottom:48}}>
        {packages.map(pkg=><div key={pkg.name} style={{background:G.surface,border:`1px solid ${pkg.highlight?hexToRgba(G.blue,0.40):'rgba(255,255,255,0.08)'}`,borderTop:`3px solid ${pkg.highlight?G.blue:'rgba(255,255,255,0.14)'}`,borderRadius:12,padding:'20px',position:'relative',boxShadow:pkg.highlight?'0 8px 32px rgba(36,88,212,0.18)':'none'}}>
          ...
          <div style={{fontWeight:700,fontSize:15,marginBottom:4,color:'#F0EFEE'}}>{pkg.name}</div>
      ...
      <h2 style={{fontWeight:700,fontSize:22,marginBottom:20,letterSpacing:'-0.3px',color:'#F0EFEE'}}>How It Works</h2>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(178px,1fr))',gap:11,marginBottom:48}}>
        {[{n:'01',ic:'📋',title:'Share your needs',desc:'Fill the form — 2 minutes.'},{n:'02',ic:'🎯',title:'We match you',desc:'rootsGTM finds the best Kosova contacts.'},{n:'03',ic:'📅',title:'Visit is planned',desc:'Meetings, tours and appointments arranged.'},{n:'04',ic:'✈️',title:'You arrive',desc:'Full schedule prepared and confirmed.'},{n:'05',ic:'📄',title:'Follow-up',desc:'Contracts, next steps and ongoing support.'}].map(step=><div key={step.n} style={{background:G.surface,border:'1px solid rgba(255,255,255,0.07)',borderRadius:10,padding:'16px'}}><div style={{fontSize:10,fontWeight:800,color:G.blue,letterSpacing:'1.5px',marginBottom:7}}>{step.n}</div><div style={{fontSize:17,marginBottom:7}}>{step.ic}</div><div style={{fontWeight:600,fontSize:13,marginBottom:4,color:'#F0EFEE'}}>{step.title}</div><div style={{fontSize:12,color:G.muted,lineHeight:1.6}}>{step.desc}</div></div>)}
      </div>
      {partnerProfiles.length>0&&<div><div className="sec-label">Network</div><h2 style={{fontWeight:700,fontSize:22,marginBottom:20,letterSpacing:'-0.3px',color:'#F0EFEE'}}>Our Partner Network</h2><div className="grid-3">{partnerProfiles.map(p=><Card2 key={p.id} p={p} lang={lang} t={t} onContact={()=>{}} onView={()=>{}}/>)}</div></div>}
    </div>
...
function Gov2({lang,t}){
  ...
  return(<div style={{background:G.bg,minHeight:'100vh',paddingTop:62}}>
    ...
    <div className="wrap2" style={{paddingTop:40,paddingBottom:48}}>
      <div className="sec-label">At a Glance</div><h2 style={{fontWeight:700,fontSize:22,marginBottom:20,letterSpacing:'-0.3px',color:'#F0EFEE'}}>{t.govFactsTitle}</h2>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(124px,1fr))',gap:9,marginBottom:44}}>
        {t.govFacts.map(([val,label],i)=><div key={label} style={{textAlign:'center',padding:'17px 12px',background:G.surface,border:'1px solid rgba(255,255,255,0.07)',borderTop:`3px solid ${CATS[i%CATS.length].color}`,borderRadius:10}}><div style={{fontWeight:800,fontSize:24,color:'#F0EFEE',letterSpacing:'-0.5px',lineHeight:1,marginBottom:5}}>{val}</div><div style={{fontSize:11,color:G.muted,fontWeight:500}}>{label}</div></div>)}
      </div>
      <div className="sec-label">Formation</div><h2 style={{fontWeight:700,fontSize:22,marginBottom:20,letterSpacing:'-0.3px',color:'#F0EFEE'}}>Company Formation</h2>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(215px,1fr))',gap:11,marginBottom:44}}>
        {t.govSteps.map((step,i)=><div key={i} style={{display:'flex',gap:11,padding:'15px',background:G.surface,border:'1px solid rgba(255,255,255,0.07)',borderLeft:`3px solid ${G.blue}`,borderRadius:10,alignItems:'flex-start'}}><div style={{width:36,height:36,borderRadius:8,background:G.blueDim,border:`1.5px solid ${G.blueBd}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:17,flexShrink:0}}>{step.ic}</div><div><div style={{fontWeight:700,fontSize:13,marginBottom:4,color:'#F0EFEE'}}>{step.t}</div><div style={{fontSize:12,color:G.muted,lineHeight:1.55,marginBottom:6}}>{step.d}</div><span style={{fontSize:10,fontWeight:700,color:G.blue,padding:'2px 6px',background:G.blueDim,border:`1px solid ${G.blueBd}`,borderRadius:4}}>⏱ {step.time}</span></div></div>)}
      </div>
      <div className="grid-2">
        <div><div className="sec-label">Benefits</div><h2 style={{fontWeight:700,fontSize:22,marginBottom:18,letterSpacing:'-0.3px',color:'#F0EFEE'}}>Why Kosova?</h2><div style={{display:'flex',flexDirection:'column',gap:10}}>{t.whyKosova.map(([ic,title,desc])=><div key={title} style={{display:'flex',gap:11,padding:'13px 15px',background:G.surface,border:'1px solid rgba(255,255,255,0.07)',borderRadius:10,alignItems:'flex-start'}}><div style={{width:34,height:34,borderRadius:8,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16,flexShrink:0}}>{ic}</div><div><div style={{fontWeight:600,fontSize:13,marginBottom:3,color:'#F0EFEE'}}>{title}</div><div style={{fontSize:12,color:G.muted,lineHeight:1.5}}>{desc}</div></div></div>)}</div></div>
        <div><div className="sec-label">Resources</div><h2 style={{fontWeight:700,fontSize:22,marginBottom:18,letterSpacing:'-0.3px',color:'#F0EFEE'}}>{t.links}</h2><div style={{display:'flex',flexDirection:'column',gap:8}}>{links.map(lk=><a key={lk.u} href={lk.u} target="_blank" rel="noopener noreferrer" style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 15px',background:G.surface,border:'1px solid rgba(255,255,255,0.07)',borderRadius:9,fontSize:13,color:'#F0EFEE',fontWeight:500,transition:'all 0.16s'}} onMouseEnter={e=>{e.currentTarget.style.borderColor=G.blue;e.currentTarget.style.color=G.blue}} onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(255,255,255,0.07)';e.currentTarget.style.color='#F0EFEE'}}>{lk.l} <span style={{color:G.blue,fontSize:12}}>→</span></a>)}</div></div>
      </div>
    </div>
  </div>)
}
...
function AdminPage2({onClose}){
  ...
  const Row=({p})=>(<div style={{display:'flex',gap:10,alignItems:'center',padding:'10px 13px',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:8,marginBottom:6}}><Lg name={p.logo||p.name} color={catColor(p.cat)} url={p.logoUrl} size={32}/><div style={{flex:1,minWidth:0}}><div style={{fontWeight:600,fontSize:13,color:'#F0EFEE'}}>{p.name}</div><div style={{fontSize:11,color:G.dim}}>{p.type} · {catLabel(p.cat,'en')} · {p.city} · {p.email}</div></div><span style={{fontSize:10,padding:'2px 6px',borderRadius:4,fontWeight:600,background:p.status==='active'||p.verified?'rgba(15,123,79,0.12)':G.blueDim,color:p.status==='active'||p.verified?'#4ade80':G.blue,border:`1px solid ${p.status==='active'||p.verified?'rgba(74,222,128,0.25)':G.blueBd}`}}>{p.status||'pending'}</span><div style={{display:'flex',gap:4}}><button onClick={()=>{setEditP(p);setEf({name:p.name,city:p.city||'',desc:p.desc?.en||p.description||'',cat:p.cat||'software',tier:p.tier||'free',website:p.website||'',phone:p.phone||''})}} style={{padding:'4px 8px',fontSize:11,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.10)',borderRadius:5,color:G.muted,cursor:'pointer'}}>✏️</button>{(p.status==='pending'||p.status==='pending_review')&&<button onClick={()=>approve(p.id)} className="btn-primary2" style={{padding:'4px 9px',fontSize:11}}>✓</button>}{(p.status==='active'||p.verified)&&<button onClick={()=>unapprove(p.id)} style={{padding:'4px 8px',fontSize:11,background:'rgba(217,119,6,0.10)',border:'1px solid rgba(217,119,6,0.22)',borderRadius:5,color:'#B45309',cursor:'pointer'}}>⊘</button>}<button onClick={()=>remove(p.id)} style={{padding:'4px 8px',fontSize:11,background:'rgba(194,53,53,0.10)',border:'1px solid rgba(194,53,53,0.22)',borderRadius:5,color:'#f87171',cursor:'pointer'}}>✕</button></div></div>)
  return(<div style={{position:'fixed',inset:0,background:'rgba(5,8,18,0.86)',zIndex:300,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'13px',overflowY:'auto',backdropFilter:'blur(8px)'}}><div style={{background:G.surface,border:'1px solid rgba(255,255,255,0.10)',borderRadius:13,width:'100%',maxWidth:840,boxShadow:'0 24px 64px rgba(0,0,0,0.60)',marginTop:10,marginBottom:10}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'15px 20px',borderBottom:'1px solid rgba(255,255,255,0.08)'}}><div style={{fontWeight:800,fontSize:15,color:'#F0EFEE'}}>⚙️ Admin — Kosova Hub</div><button onClick={onClose} className="btn-ghost2">✕ Close</button></div>
    {!auth?<div style={{padding:'42px 20px',maxWidth:290,margin:'0 auto',textAlign:'center'}}><div style={{fontWeight:700,fontSize:16,marginBottom:5,color:'#F0EFEE'}}>Admin Login</div><div style={{fontSize:13,color:G.muted,marginBottom:16}}>Enter admin password</div><input className="inp2" type="password" placeholder="Password" value={pwd} onChange={e=>setPwd(e.target.value)} onKeyDown={e=>e.key==='Enter'&&login()} style={{marginBottom:10}}/>{msg&&<div style={{color:'#f87171',fontSize:12,marginBottom:8}}>{msg}</div>}<button className="btn-primary2" style={{width:'100%'}} onClick={login}>Login →</button></div>
